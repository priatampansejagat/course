        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                $.each(dataJson.data, function(key, value) {
                    var date_custom = new Date(Date.parse(value.event.start_date));
                    course.append('<li class="course-data">' +
                        '<div class="alignright">' +
                        '<img src="' + value.event.cover_link + '" alt="image description" style="width: 80%">' +
                        '</div>' +
                        '<div class="alignleft">' +
                        ...
                        '<h3 class="list-heading">' + value.event.title + '</h3>' +
                        '<address>' + value.event.course.length + ' Course </address>' +
                        '</div>' +
                        '<div>' +
                        '<a href="myeventdetail/' + value.event.id + '" class="btn btn-warning text-uppercase">See Event Detail</a>' +
                        '</div>' +
                        '</li>');
                });
